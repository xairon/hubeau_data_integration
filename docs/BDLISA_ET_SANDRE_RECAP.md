...
### Ce qu’on intègre (mode simplifié)

| Élément | Description |
|--------|-------------|
| **Attributs TME** | Pour chaque entité : `CodeEH`, `LibelleEH`, `NiveauEH`, `NatureEH`, `MilieuEH`, `ThemeEH`, `EtatEH`, `OrigineEH`, etc. |
| **Géométrie** | **Désactivée** (pas de couche EH disponible dans le gpkg utilisé). |
...
### Sous quelle forme dans le pipeline ? (mode simplifié)

1. **Asset Dagster** `tme_entites_hydrogeo` :
   - Lit **TME.csv** (local prioritaire), sinon ZIP national.
   - Chemins locaux : racine du repo, `D:/BDLISA_V3_NATIONAL-csv(1)/CSV/TME.csv` ou `BDLISA_CSV_DIR`.
   - Charge dans `bronze.tme_entites_hydrogeo` (codes EH + attributs).
...
**Désactivé pour l’instant.** Les libellés Sandre ne sont pas chargés.
...
Non chargé en mode simplifié.

---

## 3. Synthèse : flux d’intégration (mode simplifié)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│ TME                                                                           │
│ • Source : TME.csv local (prioritaire) ou ZIP national                        │
│ • Asset : tme_entites_hydrogeo                                                │
│ • Sortie : bronze.tme_entites_hydrogeo                                        │
└─────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ dbt : stg_tme_entites                                                         │
│ • Lit : source('staging', 'tme_entites_hydrogeo')                             │
│ • Sortie : silver.stg_tme_entites (codes EH + attributs)                      │
└─────────────────────────────────────────────────────────────────────────────┘
```

-- **Bronze** : table TME uniquement.
-- **Silver** : `stg_tme_entites` = TME avec codes et attributs (sans libellés Sandre).
...
- Dans le pipeline, la table TME est :
  - en bronze : **`bronze.tme_entites_hydrogeo`** (TME.csv)
  - en silver : **`silver.stg_tme_entites`**
...
| Asset TME | `src/hubeau_pipeline/assets/bronze/tme_entites_assets.py` |
| Sources dbt | `src/dbt_hubeau/models/staging/sources.yml` (staging = schema bronze, tme_entites_hydrogeo) |
