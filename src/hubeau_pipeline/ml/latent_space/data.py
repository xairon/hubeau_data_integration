...
def load_piezo_series(pg, min_days: int = 540) -> tuple[dict[str, np.ndarray], dict[str, list]]:
    """Load eligible piezo series from Gold.

    No recency filter — inactive stations with long records are valuable
    for self-supervised contrastive learning (more diversity).
    min_days=540 (~1.5 years) guarantees at least 2 windows of 365 days.
...
def load_hydro_series(pg, min_days: int = 540) -> tuple[dict[str, np.ndarray], dict[str, list]]:
