...
# Motifs de rejet possibles en 4e position de fit_glo_detailed (tuple canonique,
# à importer plutôt qu'à redupliquer côté appelants).
GLO_REJECT_REASONS = ("n_insuffisant", "l2_degenere", "k_hors_domaine", "alpha_invalide")
...
def fit_glo_detailed(samples):
    """Ajuste une logistique généralisée (GLO, Hosking) par L-moments et expose
    en 4e position le motif de rejet (None si succès, sinon un motif de
    GLO_REJECT_REASONS).
...
def compute_spei_glo(d_cumul, glo_alpha, glo_k, glo_xi):
    ...
    Support borné (miroir de l'ancienne log-logistique, bornée en bas) : pour
    k > 0 le support est borné EN HAUT par x < ξ + α/k — un mois exceptionnellement
    humide peut donc sortir en NaN sur ces mailles ; pour k < 0 le support est
    borné EN BAS par x > ξ + α/k (déficit extrême → NaN). Sans gravité pour un
    indice de sécheresse (la queue pertinente est l'autre), mais à garder en tête
    pour l'interprétation des NaN produits.
    """
    ...
    if near_zero.any():
        # expit(y) = 1/(1+exp(-y)), numériquement stable pour |y| grand
        # (évite l'overflow de np.exp sur les cas extrêmes de |x-ξ|/α).
        cdf[near_zero] = expit((x[near_zero] - xi[near_zero]) / a[near_zero])
