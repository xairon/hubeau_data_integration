...
# Space-specific hyperparameters
# Multi: 4 input dims, standard TS2Vec/SoftCLT paper config
...
def _cluster_and_viz(context, pg, domain: str, id_col: str, space: str):
    ...
    # Space-specific UMAP pre-reduction + HDBSCAN params
    # Uni: lower intrinsic dim → smaller UMAP target + smaller min_cluster_size
    # Multi: standard config
    if space == "uni":
        wide_cfg = {"umap_dims": 5, "umap_n_neighbors": 10, "umap_min_dist": 0.1,
                    "min_cluster_size": 15, "min_samples": 5}
        fine_cfg = {"umap_dims": 5, "umap_n_neighbors": 8, "umap_min_dist": 0.0,
                    "min_cluster_size": 10, "min_samples": 3}
    else:
        wide_cfg = {"umap_dims": 15, "umap_n_neighbors": 20, "umap_min_dist": 0.1,
                    "min_cluster_size": 25, "min_samples": 10}
        fine_cfg = {"umap_dims": 10, "umap_n_neighbors": 15, "umap_min_dist": 0.0,
                    "min_cluster_size": 10, "min_samples": 5}

    # --- Config 1: Wide clusters (default) — fewer, larger clusters ---
    context.log.info(f"Config 1: Wide clusters (default) for {domain}/{space} — {wide_cfg}")
    wide = cluster_and_store(
        pg, domain, id_col,
        is_default=True,
        tune=False,
        min_cluster_size=wide_cfg["min_cluster_size"],
        min_samples=wide_cfg["min_samples"],
        umap_dims=wide_cfg["umap_dims"],
        umap_n_neighbors=wide_cfg["umap_n_neighbors"],
        umap_min_dist=wide_cfg["umap_min_dist"],
        space=space,
    )
    context.log.info(
    ...
    context.log.info(f"Config 2: Fine-grained clusters for {domain}/{space} — {fine_cfg}")
    fine = cluster_and_store(
        pg, domain, id_col,
        is_default=False,
        tune=False,
        min_cluster_size=fine_cfg["min_cluster_size"],
        min_samples=fine_cfg["min_samples"],
        umap_dims=fine_cfg["umap_dims"],
        umap_n_neighbors=fine_cfg["umap_n_neighbors"],
        umap_min_dist=fine_cfg["umap_min_dist"],
        space=space,
    )
    context.log.info(
