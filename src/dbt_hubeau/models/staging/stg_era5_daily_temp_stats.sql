{% set reprocess_from_ts = var('era5_daily_temp_reprocess_from_timestamp', none) %}
{{
  config(
...
-- Staging model for ERA5 daily temperature statistics
-- Source: bronze.era5_daily_temp_stats
-- Silver: typage, déduplication, PostGIS. Incremental append (safe dedup on hypertables).
-- Primary Key: latitude + longitude + time. Filtre incrémental: time > max(time).

WITH source AS (
    SELECT * FROM {{ source('staging', 'era5_daily_temp_stats') }}
    WHERE time IS NOT NULL
      AND latitude IS NOT NULL
      AND longitude IS NOT NULL
      AND t2m_mean IS NOT NULL
    {% if is_incremental() %}
...
deduplicated AS (
    ...
    SELECT DISTINCT ON (ROUND({{ cast_silver_numeric('latitude') }}, 1), ROUND({{ cast_silver_numeric('longitude') }}, 1), {{ cast_silver_timestamp('time') }})
        ...
        {{ cast_silver_numeric('t2m_mean') }} AS t2m_mean,
        {{ cast_silver_numeric('t2m_min') }} AS t2m_min,
        {{ cast_silver_numeric('t2m_max') }} AS t2m_max,
